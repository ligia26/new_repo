...
export class AuthService{
    ...
    login2FA(authenticationCode){
        return this.http.post("/bestinform/login2FA?authenticationCode=" +authenticationCode,{})
        .pipe(
    ...
    refreshToken() {
        return this.http
            .post('/bestinform/refreshToken?refreshToken=' + this.getRefreshToken(), {})
            .pipe(
                tap((tokens) => {
                    this.storeTokens(tokens);
                }),
    ...
    loginWithFacebook(role?: string, email?: string, token?: string) {
        return  this.http.post("/bestinform/loginWithFacebook?role=client", {accessToken: token}).pipe(
