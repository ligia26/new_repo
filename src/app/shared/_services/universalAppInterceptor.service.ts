...
@Injectable()
export class UniversalAppInterceptor
    implements HttpInterceptor
{
    ...
    intercept(req: HttpRequest<any>, next: HttpHandler) {
        ...
        if (req.url.startsWith("/bestinform")) {
            if (!req.url.startsWith("/bestinform/login") &&
        ...
        return next.handle(req).pipe(
            catchError((error) => {
                ...
                if (error instanceof HttpErrorResponse && error.status === 401 && error.error.reason === 'tokenExpired') {
                    return this.handleTokenExpiredError(req, next);
                } else if  (error instanceof HttpErrorResponse && error.status === 401 && error.error.reason === 'notLoggedIn') {
