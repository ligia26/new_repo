...
export class ReportsService {
    // fara port aici
    private url = 'https://bestinform.eu/jasperserver/rest_v2/login';
    ...
    loginJasper(username: string, password: string): Observable<HttpResponse<any>> {
        const formData = new URLSearchParams();
        formData.set('j_username', username);
        formData.set('j_password', password);

        return this.http.post<any>(this.url, formData.toString(), {
            ...this.options,
            observe: 'response'
        })
            .pipe(
